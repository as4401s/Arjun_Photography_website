...
**Primary website:** `https://www.ourtravelphotobook.com/`. Cloudflare provides DNS and registration; Netlify provides hosting and automatic HTTPS. The bare domain redirects to www, and the Netlify hostname has a path-preserving permanent redirect configured. See `docs/DOMAIN.md` for setup and release checks.
...
`SITE_URL` in `netlify.toml` sets canonical and social preview URLs for Netlify. Update it if the public domain changes. The primary canonical URL in `data/site.json` matches Netlify, so the GitHub copy points search engines to the same primary site. Change both values when adopting a custom domain. For a manual Netlify upload, build locally and upload `dist/`.
...
Homepage highlights live at `/`; destinations live at `destinations.html`, with crawlable country pages such as `switzerland-photography.html`. Older `destinations.html#country=Switzerland` links remain supported. Both pages are generated from the shared HTML template.
...
Search metadata, country pages, `robots.txt`, and the image sitemap are generated by `scripts/seo.py` during every build. See `docs/SEO.md` and `docs/DOMAIN.md`. Activate `.venv` or use `.venv/bin/python` so local imports and builds use the patched dependencies in `requirements.txt`.
