...
For subsequent releases, publish only when Arjun explicitly requests it. Verify valid HTTPS on both custom names, path-preserving redirects from the bare and Netlify hostnames, canonical URLs, sitemap, security headers, and representative image/navigation requests after publication. Search Console and Bing property verification remain account setup tasks; this domain connection does not configure them.
...
11. Add the purchased domain as a **Domain property** in [Google Search Console](https://search.google.com/search-console/), copy its verification TXT record into Cloudflare DNS, and submit `https://www.ourtravelphotobook.com/sitemap.xml` after publication. Add the site to [Bing Webmaster Tools](https://www.bing.com/webmasters/) too. Neither account has been configured by this code change.
